...
4. Trace values through boundary -> parser -> domain/service -> adapter -> response.
5. Build candidate findings only when proof exists.
6. Try to disprove each finding by checking surrounding code and local convention.
7. Output findings ordered by severity.
