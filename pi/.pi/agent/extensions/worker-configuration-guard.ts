...
const SHELL_DIRECT_MUTATION_COMMANDS = new Set([
  "rm",
  "tee",
  "touch",
  "truncate",
...
export function findWorkerConfigurationMutation(
  ...
  for (const segment of segments) {
    ...
    if (
      ...
      operands.some(
        (operand) =>
          operand.startsWith("of=") && isProtectedPath(operand.slice(3)),
      )
    ) {
      return "direct-file-command";
    }
...
export default function (pi: ExtensionAPI) {
  pi.on("tool_call", (event, ctx) => {
    ...
    if (findWorkerConfigurationMutation(event.input.command) === undefined)
      return;
