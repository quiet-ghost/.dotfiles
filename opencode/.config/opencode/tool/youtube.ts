...
function parseYouTubeUrl(rawUrl: string): VideoRef {
  ...
  if (!videoId && url.searchParams.has("list")) {
    throw new Error("Playlist-only URLs are not supported yet. Paste a video URL.");
  }
...
function chooseCaption(
  ...
  return chooseCaptionFromMap(automaticCaptions, requestedLanguage, "automatic");
}
...
async function fetchYtDlpMetadata(canonicalUrl: string): Promise<Record<string, unknown>> {
  ...
  if (!record) throw new Error("yt-dlp returned metadata in an unexpected shape.");
  return record;
}
...
function chooseVttFile(files: string[], language: string): string {
  ...
  const base = requested.split("-")[0] ?? requested;
  const preferred = files.find((file) => file.toLowerCase().includes(`.${requested}.vtt`));
  if (preferred) return preferred;
  const basePreferred = files.find((file) => file.toLowerCase().includes(`.${base}.`));
...
async function fetchCaptionWithYtDlpFile(
  ...
  try {
    ...
    const transcript = parseVtt(fs.readFileSync(chosenFile, "utf8"), includeTimestamps);
    if (!transcript) throw new Error("Downloaded caption file was empty after parsing.");
    return {
      transcript,
      language,
      source: "yt-dlp-file",
...
function mergeMetadata(
  ...
  const title = googleMetadata?.title || getString(ytdlpMetadata, "title") || ref.videoId;
  const channel =
  ...
  return {
    ...
    channel,
    channelId: googleMetadata?.channelId || getString(ytdlpMetadata, "channel_id"),
    ...
    description: googleMetadata?.description || getString(ytdlpMetadata, "description"),
    tags: googleMetadata?.tags?.length ? googleMetadata.tags : getStringArray(ytdlpMetadata, "tags"),
    viewCount: googleMetadata?.viewCount ?? getNumber(ytdlpMetadata, "view_count"),
    likeCount: googleMetadata?.likeCount ?? getNumber(ytdlpMetadata, "like_count"),
...
async function fetchGoogleMetadata(
  ...
  const durationSeconds = parseIsoDuration(getString(contentDetails, "duration"));
...
async function fetchVideo(
  ...
  if (includeGoogleMetadata) {
    try {
      googleMetadata = await fetchGoogleMetadata(ref.videoId, process.env.YOUTUBE_API_KEY);
    } catch (error) {
      warnings.push(
  ...
  try {
    caption = await fetchCaptionFromMetadata(ytdlpMetadata, language, includeTimestamps);
  } catch (error) {
    warnings.push(
...
function cacheKey(ref: VideoRef, language: string, timestamps: boolean): string {
  return [ref.videoId, language.toLowerCase(), timestamps ? "ts" : "plain"].join(":");
}
...
Use for youtube.com or youtu.be video URLs. Returns structured JSON with metadata and one transcript chunk. If totalChunks is greater than 1, call again with the same URL and the next chunkIndex until all chunks are processed. Uses yt-dlp for transcripts and optional YOUTUBE_API_KEY for richer metadata.`,
  args: {
    ...
    googleMetadata: tool.schema
      .boolean()
      .optional()
      .describe("Use YOUTUBE_API_KEY for richer metadata when available, default true"),
  },
  async execute(args, context) {
    ...
    await context.ask({
      permission: "youtube_fetch",
      patterns: [ref.canonicalUrl],
      always: [ref.canonicalUrl],
      metadata: {
        description: "Fetch YouTube metadata and transcript for note-taking.",
        videoId: ref.videoId,
      },
    });
    ...
    if (!video || args.refresh) {
      video = await fetchVideo(ref, language, includeTimestamps, includeGoogleMetadata);
      cache.set(key, video);
    }
    ...
    return {
      title: `${video.metadata.title} (${safeChunkIndex + 1}/${chunks.length})`,
      output: JSON.stringify(
        {
          ...
          transcript: {
            ...
            nextChunkIndex:
              safeChunkIndex + 1 < chunks.length ? safeChunkIndex + 1 : undefined,
            text: chunk,
          },
