...
const cvPath = paths.cv;
if (!existsSync(cvPath)) {
  errors.push(`cv.md not found at ${cvPath}. Create it with your CV in markdown format.`);
} else {
...
if (!existsSync(profilePath)) {
  errors.push(`config/profile.yml not found at ${profilePath}. Copy from ${paths.profileExample} and fill in your details.`);
} else {
...
const portalsPath = paths.portals;
if (!existsSync(portalsPath)) {
  warnings.push(`portals.yml not found at ${portalsPath}. Copy from ${paths.portalsExample} when you're ready to scan for jobs.`);
}

// 3. Check for hardcoded metrics in prompt files
const filesToCheck = [
  { path: paths.sharedMode, name: '_shared.md' },
  { path: join(paths.packageRoot, 'batch', 'batch-prompt.md'), name: 'batch-prompt.md' },
];
...
const digestPath = paths.articleDigest;
