...
Dos modos de uso: conductor interactivo con OpenCode o standalone para URLs ya recolectadas.
...
OpenCode conductor
  ...
  │    └─► opencode worker → report .md + PDF + tracker-line
  │
...
Cada worker es un `opencode run` hijo con contexto limpio. El conductor solo orquesta.
...
4. **Para cada URL pendiente**:
   ...
   d. Ejecutar via Bash:
      ```bash
      opencode run --dir ~/.local/share/career-ops \
        --file /home/ghost/.dotfiles/opencode/.config/opencode/career-ops/batch/batch-prompt.md \
