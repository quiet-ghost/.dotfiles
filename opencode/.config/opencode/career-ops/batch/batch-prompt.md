...
Eres un worker de evaluación de ofertas de empleo for the candidate (read name from `{{DATA_DIR}}/config/profile.yml`). Recibes una oferta (URL + JD text) y produces:
...
|---------|---------------|--------|
| cv.md | `{{DATA_DIR}}/cv.md` | SIEMPRE |
| llms.txt | `{{DATA_DIR}}/llms.txt (if exists)` | SIEMPRE |
| article-digest.md | `{{DATA_DIR}}/article-digest.md` | SIEMPRE (proof points) |
| i18n.ts | `{{DATA_DIR}}/i18n.ts (if exists, optional)` | Solo entrevistas/deep |
| cv-template.html | `{{PACKAGE_DIR}}/templates/cv-template.html` | Para PDF |
| generate-pdf.mjs | `{{PACKAGE_DIR}}/generate-pdf.mjs` | Para PDF |
...
Read `{{DATA_DIR}}/cv.md`. Ejecuta TODOS los bloques:
...
```
{{DATA_DIR}}/reports/{{REPORT_NUM}}-{company-slug}-{{DATE}}.md
```
...
**PDF:** `{{DATA_DIR}}/output/cv-candidate-{company-slug}-{{DATE}}.pdf`
**Batch ID:** {{ID}}
...
11. Genera HTML completo desde template (lee `{{PACKAGE_DIR}}/templates/cv-template.html`)
12. Escribe HTML a `/tmp/cv-candidate-{company-slug}.html`
13. Ejecuta:
```bash
node {{PACKAGE_DIR}}/generate-pdf.mjs \
  /tmp/cv-candidate-{company-slug}.html \
  {{DATA_DIR}}/output/cv-candidate-{company-slug}-{{DATE}}.pdf \
  --format={letter|a4}
```
...
```
{{DATA_DIR}}/batch/tracker-additions/{{ID}}.tsv
```
...
Donde `{next_num}` se calcula leyendo la última línea de `{{DATA_DIR}}/data/applications.md`.
