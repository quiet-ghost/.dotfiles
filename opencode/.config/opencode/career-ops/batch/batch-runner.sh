...
# career-ops batch runner — standalone orchestrator for opencode workers
# Reads batch-input.tsv, delegates each offer to an opencode worker,
# tracks state in batch-state.tsv for resumability.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
BATCH_DIR="$SCRIPT_DIR"
DATA_DIR="${CAREER_OPS_DATA_DIR:-$HOME/.local/share/career-ops}"
INPUT_FILE="$DATA_DIR/batch/batch-input.tsv"
STATE_FILE="$DATA_DIR/batch/batch-state.tsv"
PROMPT_FILE="$BATCH_DIR/batch-prompt.md"
LOGS_DIR="$DATA_DIR/batch/logs"
TRACKER_DIR="$DATA_DIR/batch/tracker-additions"
REPORTS_DIR="$DATA_DIR/reports"
APPLICATIONS_FILE="$DATA_DIR/data/applications.md"
LOCK_FILE="$DATA_DIR/batch/batch-runner.pid"
...
usage() {
  cat <<'USAGE'
career-ops batch runner — process job offers in batch via OpenCode workers
Uses your default OpenCode model and command harness.
...
check_prerequisites() {
  ...
  mkdir -p "$LOGS_DIR" "$TRACKER_DIR" "$REPORTS_DIR" "$DATA_DIR/data" "$DATA_DIR/output" "$DATA_DIR/jds"
}
...
process_offer() {
  ...
  local prompt
  prompt="Procesa esta oferta de empleo. Ejecuta el pipeline completo: evaluacion A-F + report .md + PDF + tracker line."
  ...
  sed \
    ...
    -e "s|{{DATE}}|${date}|g" \
    -e "s|{{ID}}|${id}|g" \
    -e "s|{{PACKAGE_DIR}}|${PROJECT_DIR}|g" \
    -e "s|{{DATA_DIR}}|${DATA_DIR}|g" \
    "$PROMPT_FILE" > "$resolved_prompt"

  # Launch opencode worker with the standalone router command.
  local exit_code=0
  opencode run \
    --dir "$DATA_DIR" \
    --print-logs \
...
merge_tracker() {
  ...
  CAREER_OPS_DATA_DIR="$DATA_DIR" node "$PROJECT_DIR/merge-tracker.mjs"
  echo ""
  echo "=== Verifying pipeline integrity ==="
  CAREER_OPS_DATA_DIR="$DATA_DIR" node "$PROJECT_DIR/verify-pipeline.mjs" || echo "⚠️  Verification found issues (see above)"
}
