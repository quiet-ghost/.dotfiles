...
const DEFAULT_PATTERNS: CloakRuleConfig[] = [
  {
    filePattern: "**/*",
    cloakPattern: {
      pattern: String.raw`("(?:token|access|refresh|accessToken|refreshToken|apiKey|secret|password|clientSecret|idToken|sessionToken|authorization)"\s*:\s*")[^"]+`,
      replace: "$1",
      flags: "i",
    },
  },
  {
    filePattern: ["**/.env*", "**/*.vars*"],
    cloakPattern: {
  ...
  },
  {
    ...
    cloakPattern: {
      pattern: String.raw`^(\s*(?:token|api_key|secret|password)\s*=\s*).+$`,
      replace: "$1",
      flags: "i",
    },
  },
...
export function loadState(
  ...
  try {
    ...
    const config: CloakConfig = {
      ...DEFAULT_CONFIG,
      ...parsed,
      patterns: parsed.patterns ?? DEFAULT_PATTERNS,
    };
