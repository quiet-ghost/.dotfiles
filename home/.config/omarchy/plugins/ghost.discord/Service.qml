...
Item {
  ...
  // Vesktop's discord-ipc-0 is arRPC, not Discord's voice socket, so leave the bridge off.
  Rpc {
    id: bridge
    active: false
  }
  ...
  // The launcher's own path, so Vesktop lands in app-graphical.slice, not the compositor's.
  function launch() {
    if (!installed) return
    Util.execDetached("uwsm-app -- gtk-launch vesktop.desktop")
    settle()
  }
  ...
  // ps always reports one vesktop without --type=, so no main pid means the parse failed.
  function quit() {
    if (!running) return
    if (mainPid === 0) {
      lastError = "Could not find the main Vesktop process to quit"
      return
    }
  ...
  Process {
    id: statusProcess
    running: false
    command: ["ps", "-C", "vesktop", "-o", "pid=,rss=,args="]
    ...
    onExited: function (exitCode) {
      if (exitCode === 0 || exitCode === 1) {
        ...
        root.lastError = String(statusStderr.text || "") || "Could not read Vesktop processes"
      }
    }
  }
