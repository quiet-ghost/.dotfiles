...
function findEntry(applications) {
  ...
  for (var i = 0; i < list.length; i++) {
    var entry = list[i]
    if (entry && String(entry.startupClass || "").toLowerCase() === "vesktop") return entry
  }
  return null
}
...
// hyprctl reports both class and initialClass as exactly "vesktop" here.
function matchWindows(toplevels) {
  var list = toplevels || []
  var out = []
  for (var i = 0; i < list.length; i++) {
    if (String(toplevelClass(list[i])).toLowerCase() === "vesktop") out.push(list[i])
  }
  return out
}
...
function isOwnedByDiscord(node) {
  return String(nodeProps(node)["application.process.binary"] || "") === "vesktop"
}
...
// lines look like "3957804 576760 /usr/lib/vesktop/vesktop --type=renderer"
...
// titles read "• Discord | Friends" or "#general | Server - Discord"
function windowTitle(toplevel) {
  var title = toplevel && toplevel.title ? String(toplevel.title) : ""
  if (title.indexOf("• ") === 0) title = title.slice(2)
  var suffix = " - Discord"
