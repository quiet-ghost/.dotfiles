...
// Pull the OAuth token out of wrangler's config. Wrangler writes plain
// `key = "value"` lines, so a line-oriented match is enough and avoids
// shipping a TOML parser for three fields.
function parseWranglerConfig(text) {
  var out = { token: "", expiresMs: 0, scopes: [] }
  var lines = String(text || "").split("\n")
  for (var i = 0; i < lines.length; i++) {
    var line = lines[i]
    var token = line.match(/^\s*oauth_token\s*=\s*"([^"]*)"/)
    if (token) { out.token = token[1]; continue }
    var expires = line.match(/^\s*expiration_time\s*=\s*"([^"]*)"/)
    if (expires) {
      var parsed = Date.parse(expires[1])
      out.expiresMs = isFinite(parsed) ? parsed : 0
      continue
    }
    var scopes = line.match(/^\s*scopes\s*=\s*\[(.*)\]/)
    if (scopes) {
      var found = scopes[1].match(/"([^"]+)"/g)
      out.scopes = found ? found.map(function(s) { return s.slice(1, -1) }) : []
    }
  }
  return out
}
