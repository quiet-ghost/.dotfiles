...
Item {
  ...
  readonly property string projectsRoot: {
    ...
    return configured !== "" ? configured : Quickshell.env("HOME") + "/dev"
  }
  ...
  function notify(title, body) {
    Quickshell.execDetached(["omarchy-notification-send", String(title), String(body || "")])
  }
  ...
  function openUrl(url) {
    ...
    var launcher = target.indexOf("https://dash.cloudflare.com/") === 0
      ? "omarchy-launch-webapp"
      : "omarchy-launch-browser"
    Quickshell.execDetached([launcher, target])
  }

  // Mutating wrangler commands run in a visible terminal rather than silently
  // in the background: you get the real output, the confirmation prompts, and
  // somewhere to hit Ctrl-C.
  function runInTerminal(command) {
    Quickshell.execDetached(["omarchy-launch-floating-terminal-with-presentation", String(command)])
  }
