...
"chrome-devtools": {
  "type": "local",
  "command": ["npx", "-y", "chrome-devtools-mcp@<reviewed-version>"]
}
```

Resolve and review the current release before suggesting a concrete version. Do not configure a mutable `@latest` package for automatic execution.
