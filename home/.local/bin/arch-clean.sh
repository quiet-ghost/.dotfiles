...
if [ $exit_code -ne 0 ]; then
    if echo "$output" | grep -q "conflicting files"; then
        if echo "$output" | grep -q "linux-firmware-nvidia"; then
            echo "Nvidia firmware conflict detected, resolving..."
            pacman -Syu --noconfirm --overwrite '/usr/lib/firmware/nvidia/*'
        else
            echo "Other file conflicts detected, may need manual resolution:"
            echo "$output"
            exit 1
        fi
    else
        echo "Non-conflict error occurred:"
        echo "$output"
        exit 1
    fi
else
